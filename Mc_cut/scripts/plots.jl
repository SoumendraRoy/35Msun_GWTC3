...
population_params = [:R, :rhm, :alphahm, :alpha, :mtr, :mbhmax, :sigma, :beta, :mul, :muh, :sigmal, :sigmah, :lambda, :zp, :kappa, :alpha_chi_eff, :beta_chi_eff, :Neff_sel]
cosmo_params = [:h, :Ω_M]
...
function log_dNdm_from_chain_sample(trace, draw, chain; mgrid=10:0.25:100)
    ...
    BumpCosmologyGWTC3.make_combined_log_dNdm(p.log_rhm[draw=d, chain=c], p.alphahm[draw=d, chain=c], p.alpha[draw=d, chain=c], p.mtr[draw=d, chain=c], p.mbhmax[draw=d, chain=c], p.sigma[draw=d, chain=c]; mgrid=mgrid)
end
...
function log_dN_from_chain_sample(trace, draw, chain)
    ...
    log_dN = BumpCosmologyGWTC3.make_log_dNdm1dqdchiedVdt(p.log_rhm[draw=d, chain=c], p.alphahm[draw=d, chain=c], p.alpha[draw=d, chain=c], p.mtr[draw=d, chain=c], p.mbhmax[draw=d, chain=c], p.sigma[draw=d, chain=c], p.beta[draw=d, chain=c], p.mul[draw=d, chain=c], p.muh[draw=d, chain=c], p.sigmal[draw=d, chain=c], p.sigmah[draw=d, chain=c], p.lambda[draw=d, chain=c], p.zp[draw=d, chain=c], p.kappa[draw=d, chain=c])
    log_dN
end
...
function q_chieff_corner_plot(trace;
    ...
    blue_colormap = cgrad(:blues)  # built-in in Makie, or customize if needed
...
function frac_chirp_mass_bounds(df, mc_min, mc_max)
    mcs = df[!, :chirp_mass_source]
    p_in = sum((mcs .>= mc_min) .& (mcs .<= mc_max)) / size(df, 1)

    p_in
end

function m1m2_kde_plot(df; mc_min = chirp_mass(20.0, 20.0), mc_max=chirp_mass(50.0, 50.0), include_legend=false, p_cut=[0.9, 0.5, 0.1], p_cut_eventlist=0.0, rng=Random.default_rng(), far_thresh=1, alpha_factor=0.5, level=0.1, draw_cut_lines=false)
    ...
    f = Figure()
    a = Axis(f[1, 1]; xlabel=L"\mathrm{Primary~Mass~(in~} M_\odot)", ylabel=L"\mathrm{Secondary~Mass~(in~} M_\odot)", xlabelsize=22, ylabelsize=22, titlesize=24, limits=(mlow,mhigh,mlow,mhigh), title=L"\mathrm{PE~Posterior}(20-20<M_c<50-50)>%$(p_cut)", xminorgridvisible=true, xminorticksvisible=true, yminorgridvisible=true, yminorticksvisible=true)

    # Block out the region where m1 < m2
    band!(a, [mlow, mhigh], [mlow, mhigh], [mhigh, mhigh], color=(:grey, 0.25))

    # Maybe plot the cuts
    if draw_cut_lines
        qs = collect(0.01:0.01:1)
        m1s_low = mc_min .* (1 .+ qs).^(1/5) ./ qs.^(3/5)
        m2s_low = m1s_low .* qs

        m1s_high = mc_max .* (1 .+ qs).^(1/5) ./ qs.^(3/5)
        m2s_high = m1s_high .* qs

        msl = collect(mlow:0.01:mc_min*2^(1/5))

        band!(a, msl, zeros(length(msl)), msl, color=(:grey, 0.25))
        band!(a, m1s_low, mlow .* ones(length(m1s_low)), m2s_low, color=(:grey, 0.25))
        band!(a, m1s_high, m2s_high, m1s_high, color=(:grey, 0.25))
    end

    @progress for (i, evt) in enumerate(evt_groups)
        alpha = 1.0
        p = frac_chirp_mass_bounds(evt, mc_min, mc_max)
        for pc in p_cut
            if p > pc
                break
            else
...
function qchieff_kde_plot(df; mc_min = chirp_mass(20.0, 20.0), mc_max=chirp_mass(50.0, 50.0), include_legend=false, p_cut=0.5, p_cut_eventlist=0.5, rng=Random.default_rng(), far_thresh=1)
    ...
    f = Figure()
    a = Axis(f[1, 1]; xlabel=L"\mathrm{Effective~Spin,~}\chi_\mathrm{eff}", ylabel=L"\mathrm{Mass~Ratio,~}q", xlabelsize=22, ylabelsize=22, limits=(-1,1,0,1), title=L"p = %$(p_cut)", xminorgridvisible=true, xminorticksvisible=true, yminorgridvisible=true, yminorticksvisible=true)
